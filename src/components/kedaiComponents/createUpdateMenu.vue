...
export default{
  ...
  computed: {
    ...
    formattedAmountHargaPilihan() {
      return this.customInput.customMenu.map(menu =>
        this.formatRupiah(menu.harga_pilihan || "")
      );
    },
    },
  methods: {
    ...
    saveCustom() {
        ...
        if (this.typeSaveCustome === 'update') {
            ...
            this.FormData.customeKategoriMenu = this.FormData.customeKategoriMenu.filter(item => 
                item.new_kategori_id !== customeKategoriMenu.new_kategori_id
            );
            this.FormData.customMenu = this.FormData.customMenu.filter(item => 
                item.mark_id_kategori !== customeKategoriMenu.new_kategori_id
            );
            if(this.tipe == 'update'){
            ...
            if (this.tipe == 'update'){
                if(this.FormData.customeKategoriMenu.length > 0){
                    let forMarkUpdate = Math.max(...this.FormData.customeKategoriMenu.map(kategori => kategori.new_kategori_id))
        ...
        customMenu.forEach((menuItem) => {
            if (menuItem.nama_pilihan) {
                ...
                const formatedHarga = menuItem.harga_pilihan ? menuItem.harga_pilihan.replace(/\./g, "") : "";
    ...
    async submitMenu() {
        ...
        const harga = typeof formData.harga === 'number' ? formData.harga.toString() : formData.harga.replace(/\D/g, "");
        ...
        if (this.tipe === 'update') {
            ...
            data.kategori_detail_id = formData.customeKategoriMenu.map(item => item.kategori_detail_id);
            data.id_detail = formData.customMenu.map(item => item.id_pilihan);
        } else {
            data.new_kategori_id = formData.customeKategoriMenu.map(item => item.new_kategori_id).filter(Boolean);
        }
    ...
    <v-row v-if="!isLoading" no-gutters>
        ...
        <v-col cols="12">
            <div class="select-group">
                <select v-model="FormData.kategoriMenu" class="select-custom rounded-5 mb-4" >
                    ...
                    <option v-for="kategori in kategoris" :key="kategori.value" :value="kategori.value">{{ kategori.text }}</option>
                </select>
        ...
        <v-col cols="6" class="px-1" >
            <div class="select-group">
                ...
                <option disabled value="">Please select one</option>
                <option v-for="status in itemsStatus" :key="status.value" :value="status.value">{{ status.text }}</option>
                </select>
        ...
        <v-col v-for="(kategori , index) in FormData.customeKategoriMenu" :key="index" cols="12">
            <v-card v-if="kategori.nama_kategori != ''" class="mx-auto my-3 p-3">
                <div class="d-flex justify-space-between">
                    <div>
                        <p class="text-h6 text-capitalize font-weight-bold mb-0"> - {{ kategori.nama_kategori }}</p>
                        <p> {{ kategori.opsi == 0 ? 'Opsional' : 'Wajib'  }} | max {{ kategori.max_pilih }}</p>
                    </div>
                    <div>
                    <v-btn size="small" icon="mdi-trash-can" color="red" class="mr-4" @click="removePickCustom(index)" >
                    </v-btn>
                ...
                <div v-for="(pilihan , index) in FormData.customMenu" :key="index">
                    <v-card-item v-if="pilihan.nama_pilihan != '' && pilihan.mark_id_kategori == kategori.new_kategori_id" class="elevation-2 mb-2">
                    <v-row>
                        <v-col cols="6">
                        ...
                        <v-col cols="5" class="text-end">
                            <v-btn size="small" icon="mdi-trash-can" color="red" class="mr-4" @click="removePickCustomPilihan(index)" >
                            </v-btn>
                        </v-col>
                    </v-row>
                    </v-card-item>
          ...
          <v-toolbar-title>Menambahkan Custom menu</v-toolbar-title>
            ...
            <v-container>
                <v-row no-gutters>
                    ...
                    <v-col cols="6">
                        <div class="select-group">
                            <select v-model="customInput.customeKategoriMenu.opsi" class="select-custom rounded-5 mb-4" >
                                <option disabled value="">Please select one</option>
                                <option v-for="opsi in itemsOpsi" :key="opsi.value" :value="opsi.value">{{ opsi.text }}</option>
                                </select>
                                <label class="user-label">Status</label>
                        </div>
                    </v-col>
